...
def _toll_chart(lang, latest_total=None):
    ...
    if pts is None:
        ...
        # No curve when there is no series: a drawn shape that stands for
        # nothing is a fake chart (review of PR #365, 2026-09-05). The cell
        # keeps its place and says plainly what it knows — the total — and
        # what it cannot show right now.
        return (
            f'<div class="gi-cell gi-spark"><figure class="toll-chart toll-chart-fallback" '
            f'dir="{"rtl" if ar else "ltr"}" role="group" '
            f'aria-label="{head}: {_num(latest_total, lang)} — {note}">'
            f'<figcaption class="gi-lab">{label}</figcaption>'
            f'</figure></div>')
    if len(pts) < 30:
        return ""
    ar = lang == "ar"
