...
# page): before a figure animates, its box is pinned to the wider of the
# start and end renderings and the text never wraps, so digits repaint in
# place and no frame can reflow the grid, the strip or anything below.
PANEL_JS = """
...
function lockWidth(el,from,to){var keep=el.getAttribute("data-gi-val");
 setNum(el,to);var w=el.offsetWidth;setNum(el,from);
 if(el.offsetWidth>w)w=el.offsetWidth;
 el.style.minInlineSize=w+"px";el.setAttribute("data-gi-val",keep)}
function unlockWidth(el){el.style.minInlineSize=""}
function animate(el,from,to){if(RM||from===to){setNum(el,to);return}
 ...
 function step(t){var p=Math.min(1,(t-t0)/dur);p=1-Math.pow(1-p,3);
  ...
  else{setNum(el,to);unlockWidth(el)}}
